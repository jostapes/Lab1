package Lab1;

import java.util.Scanner;

public class Lab1 {

	public static void main(String[] args) {
		Scanner in = new Scanner(System.in);
		
		
		System.out.println("How many years are you planning to work?");
		int yearsLeft = in.nextInt();
	
		System.out.println("What is your expected average return on investment?");
		double annualReturn1 = in.nextDouble();
		
		System.out.println("How many years you have been retired for?");
		int yearsRetired = in.nextInt();
		
		System.out.println("What is your annual return for while retired?");
		double annualReturn2 = in.nextDouble();
		
		System.out.println("What is your expected income?");
		double requiredIncome = in.nextDouble();
		
		System.out.println("What is your expected monthly SSI?");
		double monthlySSI = in.nextDouble();
		
		
		
		
		
		
		
		
	
	}

}
